...
let multiLineInput input 
    = List.fold inputFold [ List.empty ] input |> List.rev
...
let main argv =
    let input = readLines "input.txt"
                |> Seq.toList
                |> multiLineInput
                |> List.map ((fun x -> (List.length x, List.collect Seq.toList x))
                             >> fun (x, y) -> (x, List.groupBy id y)
                             >> fun (x, y) -> (x, List.map (fun(letter, answers) -> (letter, List.length answers)) y)
                             >> fun (x, y) -> y |> List.filter (fun (_, answers) -> x = answers) |> List.length)
                // |> List.iter (printfn "%A")
                |> List.sum
                |> printfn "%d"
    // input 
